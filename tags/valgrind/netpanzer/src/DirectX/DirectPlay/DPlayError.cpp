...
char * DPlayErrorToString( HRESULT error )
 {
...
return " The session is full or an incorrect password was supplied. ";
...
return " No credentials were supplied and the CryptoAPI package (CAPI) to use for cryptography services cannot be loaded. ";

case DPERR_CANTLOADSECURITYPACKAGE :

return " The software security package cannot be loaded. ";
...
return " The capabilities of the DirectPlay object have not been determined yet. This error will occur if the DirectPlay object is implemented on a connectivity solution that requires polling to determine available bandwidth and latency. ";

case DPERR_CONNECTING :

return " The method is in the process of connecting to the network. The application should keep calling the method until it returns DP_OK"; 
...
return " An undefined error condition occurred. ";

case DPERR_INVALIDFLAGS :

return " The flags passed to this method are invalid. ";
...
return " One or more of the parameters passed to the method are invalid. ";
...
return " The session could not be opened because credentials are required and either no credentials were supplied or the credentials were invalid. ";
...
return "Returned by the IDirectPlayLobby2::Connect method if the application was not started by using the IDirectPlayLobby2::RunApplication method or if there is no DPLCONNECTION structure currently initialized for this DirectPlayLobby object. ";

case DPERR_NOTLOGGEDIN :

return " An action cannot be performed because a player or client application is not logged in. returned by the IDirectPlay3::Send method when the client application tries to send a secure message without being logged in. ";

case DPERR_OUTOFMEMORY :

return " There is insufficient memory to perform the requested operation. ";
...
return " The message being sent by the IDirectPlay3::Send method is too large. ";
...
return " The requested information could not be digitally signed. Digital signatures are used to establish the authenticity of messages. ";
...
return " The function is not available in this implementation. Returned from IDirectPlay3::GetGroupConnectionSettings and IDirectPlay3::SetGroupConnectionSettings if they are called from a session that is not a lobby session. ";

case DPERR_USERCANCEL :

return "Can be returned in two ways. 1) The user canceled the connection process during a call to the IDirectPlay3::Open method. 2) The user clicked Cancel in one of the DirectPlay service provider dialog boxes during a call to IDirectPlay3::EnumSessions. ";
