...
public:
	...
	inline bool   getDoesExist() const { return doesExist; }
	inline iXY    getPix() const { return pix; }
	inline int    getPixX() const { return pix.x; }
	inline int    getPixY() const { return pix.y; }
	inline float  getAspectXOverY() const { return float(pix.x) / float(pix.y); }
	inline float  getAspectYOverX() const { return float(pix.y) / float(pix.x); }
...
public:
    ...
    inline void blt(const Surface &dest) const { blt(dest, iXY(0, 0)); }
    inline void blt(const Surface &dest, int x, int y) const { blt(dest, iXY(x, y)); }

	void bltTrans(const Surface &dest, iXY pos) const;
    inline void bltTrans(const Surface &dest) const { bltTrans(dest, iXY(0, 0)); }
    inline void bltTrans(const Surface &dest, int x, int y) const { bltTrans(dest, iXY(x, y)); }

	void bltTransColor(const Surface &dest, iXY pos, const PIX &color) const;
    inline void bltTransColor(const Surface &dest, const PIX &color) const { bltTransColor(dest, iXY(0, 0), color); }
    inline void bltTransColor(const Surface &dest, int x, int y, const PIX &color) const { bltTransColor(dest, iXY(x, y), color); }
	...
	inline void shade(PIX color) const { shade(iRect(0, 0, pix.x, pix.y), color); }

	void blendIn(const Surface &source, iXY min, ColorTable &colorTable) const;
	inline void blendIn(const Surface &source, ColorTable &colorTable) const { blendIn(source, iXY(0, 0), colorTable); }
	inline void blendIn(const Surface &source, int x, int y, ColorTable &colorTable) const { blendIn(source, iXY(x, y), colorTable); }
	...
	inline void drawRect(int x1, int y1, int x2, int y2, const PIX &color) const { drawRect(iRect(x1, y1, x2, y2), color); }
	inline void drawRect(const iXY &min, const iXY &max, const PIX &color) const { drawRect(iRect(min.x, min.y, max.x, max.y), color); }
	inline void drawRect(const PIX &color) const { drawRect(iRect(0, 0, pix.x, pix.y), color); }
