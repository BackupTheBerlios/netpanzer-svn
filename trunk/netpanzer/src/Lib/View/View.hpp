...
public:
	...
	enum { MAX_WINDOW_CLIENT_XSIZE = 632 };
	enum { MAX_WINDOW_CLIENT_YSIZE = 458 };

	inline int getPressedButton() { return pressedButton; }
	inline int getPrevPressedButton() { return prevPressedButton; }
	inline int getHighlightedButton() { return highlightedButton; }
	inline int getPrevHighlightedButton() { return prevHighlightedButton; }
...
public:
	...
	void removeAllButtons() { buttons.setNum(0); }
	...
	inline const char *getSubTitle() const { return subTitle; }

	inline int getActive() const { return status & STATUS_ACTIVE; }
	inline int getVisible() const { return status & STATUS_VISIBLE; }
	inline int getAllowMove() const { return status & STATUS_ALLOW_MOVE; }
	inline int getBordered() const { return status & STATUS_BORDERED; }
	inline int getAlwaysOnBottom() const { return status & STATUS_ALWAYS_ON_BOTTOM; }
	inline int getShowStatus() const { return status & STATUS_DISPLAY_STATUS_BAR; }
	inline int getResize() const { return status & STATUS_ALLOW_RESIZE; }
	inline int getScrollBar() const { return status & STATUS_SCROLL_BAR; }
	inline int getAllowAutoResize() const { return status & STATUS_ALLOW_AUTO_RESIZE; }
